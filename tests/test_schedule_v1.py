...
class ScheduleV1Test(TestCase):
    ...
    def test_legacy_pending_proposal_is_read_only_and_not_active_ui_state(self) -> None:
        database = Path(self.temporary.name) / "chronos.sqlite3"
        legacy = self.proposals.create(
            "明天下午安排60分钟写代码",
            now=datetime(2026, 8, 3, 9, 0, tzinfo=self.zone),
        )
        operations = OperationStore(SQLiteAgentOperationRepository(database))
        router = V1Router(
            self.schedule,
            self.proposals,
            chronos_log=ChronosLogService(SQLiteChronosLogRepository(database)),
            operation_store=operations,
            projections=ProjectionService(operations),
        )

        _, listed = router.dispatch("GET", "/api/v1/proposals")
        item = listed["data"]["proposals"][0]
        _, fetched = router.dispatch(
            "GET", f"/api/v1/proposals/{legacy['proposal_id']}"
        )
        _, projections = router.dispatch("GET", "/api/v1/timeline-projections")
        _, log = router.dispatch("GET", "/api/v1/chronos-log")

        self.assertEqual(item["source"], "legacy")
        self.assertTrue(item["read_only"])
        self.assertFalse(item["requires_confirmation"])
        self.assertEqual(fetched["data"]["source"], "legacy")
        self.assertEqual(projections["data"]["projections"], [])
        self.assertEqual(log["data"]["pending_count"], 0)
