...
from chronos.agent.meaning import Interpreter, Item, Meaning
...
class BoundaryTest(TestCase):
    def test_interpreter_contract_accepts_items_and_returns_meaning(self) -> None:
        annotation = Interpreter.interpret.__annotations__
        self.assertEqual(annotation["items"], "tuple[Item, ...]")
        self.assertEqual(annotation["return"], "tuple[Meaning, ...]")
